...
const token =
  "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..XWlUBIbNha6xPW-hCmKgK7e30mmdRGpxFirv6nJYJiW7FnAZ6-crVBIVDifTPG3OvZ0iqWDPKSo8-8cNOdPbmH3wa4umuHwj0dD4Dsugysp_eLRCVcSvcFacZ9j5bcLmPbsxMzDMLCPAGHFJ7Xp_0gKDt-9j-dMnDCMIg2mCMtA8AX4v5xqfr5DU1nNax9p8-FZdTs8PruPQLGfr4WlBZR631wLc0z2f7kvC-gTP7VCJ2k23zGatW34uVPIhMRqOdMyTUWTZFaCnWWNVc3u_j5BvPkbVTeN_rjP0uslvbZ2YaUjs3XvsJDLX9NfVOAEKItPUGBaY9GQaEMQzKBYIIQ";
...
export const shouldBlock = (receiver_rules, caller_details) => {
  ...
  return (
    blockedCountries.has(callerCountry) ||
    (blockBusiness && isCallerBusiness )|| (blockSpam && isCallerSpam) 
  );
};
...
export const getRedactedNumber = (
  caller_details,
  receiver_details,
  isMyContact
) => {
  ...
  var isReceiverForeign =
    caller_details["country"] !== receiver_details["country"];

  if (!isMyContact) {
    const response = redact(result, notMyContacts, maxRedaction);

    result = response[0];
    maxRedaction = response[1];
  }
  if (isReceiverBusiness) {
    const response = redact(result, business, maxRedaction);

    result = response[0];
    maxRedaction = response[1];
  }
  if (isReceiverForeign) {
    const response = redact(result, foreign_num, maxRedaction);
    result = response[0];
    maxRedaction = response[1];
  }
...
const redact = (number, redactionType, maxRedaction) => {
  ...
  if (redactionType === "REDACTED") {
    return [redactNumber(number), redactionType];
  }
};

const maskNumber = (phoneNumber) => {
  var result = phoneNumber[0] + phoneNumber[1] + phoneNumber[2];
  for (let i = 3; i < phoneNumber.length - 2; i++) {
    result += "*";
  }
  result +=
    phoneNumber[phoneNumber.length - 2] + phoneNumber[phoneNumber.length - 1];
